...
# A key of this deployment's own, not the shard baked into the image and shared
# across customers. Only before the store exists: the key is what the rows are
# sealed with, so changing it later makes all of them unreadable.
take_a_shard_of_our_own() {
  ...
  # No flag: an older cluster, which may already hold sealed rows.
  ...
  # 204 means no shard on the record; the baked one still works.
  [[ -n "$shard" ]] || return 0
